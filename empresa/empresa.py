...
class PredictEmprestimo(object):
    ...
    def data_preparation(self, df3):
        print('preparation')
        df3['idade'] = self.idade.transform(df3[['idade']].values)
        df3['renda'] = self.renda.transform(df3[['renda']].values)
        df3['tempo_emprego'] = self.tempo_emprego.transform(df3[['tempo_emprego']].values)
        df3['valor_emprestimo'] = self.valor_emprestimo.transform(df3[['valor_emprestimo']].values)
        df3['taxa_juros_emprestimo'] = self.taxa_juros_emprestimo.transform(df3[['taxa_juros_emprestimo']].values)
        df3['relacao_emprestimo_renda'] = self.relacao_emprestimo_renda.transform(df3[['relacao_emprestimo_renda']].values)
        df3['historico_credito'] = self.historico_credito.transform(df3[['historico_credito']].values)
        df3['taxa_juros_ajustada_renda'] = self.taxa_juros_ajustada_renda.transform(df3[['taxa_juros_ajustada_renda']].values)
        df3['proporcao_emprestimo_idade'] = self.proporcao_emprestimo_idade.transform(df3[['proporcao_emprestimo_idade']].values)
        df3['proporcao_emprestimo_tempo_emprego'] = self.proporcao_emprestimo_tempo_emprego.transform(df3[['proporcao_emprestimo_tempo_emprego']].values)
        df3['proporcao_emprestimo_historico_credito'] = self.proporcao_emprestimo_historico_credito.transform(df3[['proporcao_emprestimo_historico_credito']].values)
        df3['proporcao_renda_emprestimo'] = self.proporcao_renda_emprestimo.transform(df3[['proporcao_renda_emprestimo']].values)
